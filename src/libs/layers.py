...
def dense_nonlatent(inputs,
    ...
    with tf.variable_scope(scope, "dense_nonlatent", [inputs], reuse=reuse) as sc:
        enc_out = fully_connected(inputs,
                             num_outputs,
                             activation_fn=mu_nl,
                             normalizer_fn=normalizer_fn,
                             normalizer_params=normalizer_params,
...
def deconv_nonlatent(inputs,
                  ...
                  post_trim=None,
                  scope=None):
    """a deconvolutional non latent variable layer"""
    # normalizer is disabled for now
    assert(normalizer_fn is None and normalizer_params is None)

    with tf.variable_scope(scope, "deconv_nonlatent", [inputs], reuse=reuse) as sc:
        dec_out = conv2d_transpose(inputs,
                              num_outputs=num_outputs,
                              kernel_size=kernel_size,
                              stride=stride,
                              padding=padding,
                              ...
                              scope="dec_out")
        if post_trim:
            print("before cropping: %s" % (dec_out.shape.as_list()))
            if data_format == "NCHW":
                dec_out = dec_out[..., post_trim[0], post_trim[1]]
                #logvar = logvar[...,  post_trim[0], post_trim[1]]
            elif data_format == "NHWC":
                dec_out = dec_out[..., post_trim[0], post_trim[1], :]
                #logvar = logvar[...,  post_trim[0], post_trim[1], :]
