...
template<typename Real>
__global__
static void _square_relu(Real* mat, Real floor_val, MatrixDim d) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;  // col index
  int j = blockIdx.y * blockDim.y + threadIdx.y;  // row index
  int index = i + j * d.stride;
...
static void _diff_square_relu(Real* mat, MatrixDim d) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;  // col index
  int j = blockIdx.y * blockDim.y + threadIdx.y;  // row index
  int index = i + j * d.stride;
  if (i < d.cols && j < d.rows)
    mat[index] = (mat[index] > 0.0 ? 1.0/2*sqrt(mat[index]) : 0.0);
}
...
void cudaF_diff_square_relu(dim3 Gr, dim3 Bl, float* mat, MatrixDim d) {
  _diff_square_relu<<<Gr,Bl>>>(mat, d);
}
...
void cudaF_square_relu(dim3 Gr, dim3 Bl, float* mat, float floor_val, MatrixDim d) {
  _square_relu<<<Gr,Bl>>>(mat, floor_val, d);
}
...
void cudaD_diff_square_relu(dim3 Gr, dim3 Bl, double* mat, MatrixDim d) {
  _diff_square_relu<<<Gr,Bl>>>(mat, d);
}
...
void cudaD_square_relu(dim3 Gr, dim3 Bl, double* mat, double floor_val, MatrixDim d) {
  _square_relu<<<Gr,Bl>>>(mat, floor_val, d);
}
