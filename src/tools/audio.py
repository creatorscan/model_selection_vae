...
class MelBank(object):
    def __init__(self, 
        ...
        self.mel_windows = [mel_freqs[i:i+3] for i in range(self.num_bins)]
...
def stft_index(wave, frame_size_n, frame_starts_n, fft_size=None, win=None):
    ...
    spec = np.asarray([scipy.fft(wave[np.int(n):np.int(n)+frame_size_n]*win, n=fft_size)[:fft_size//2+1] \
